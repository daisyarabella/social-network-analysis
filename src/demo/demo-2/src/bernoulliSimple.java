...
{   
    ...
    {
    	File timestepData = new File("../../../data/BStimestepData.csv"); // file to record timestep data for plotting simple line graph
    	File linearEqs = new File("../../../data/BSlinearEqs.csv"); // file to be read by python to calculate p and q. Includes S(t+1) data, and coefficients of a, b, c
    	...
    	linearfw.write("Iteration,GraphNo,Stadd1,aCo,bCo,cCo,edgeProb,p\n");
        ...
        for (int iteration=1; iteration<=1; iteration++) {	
        	for (double edgeProb = 0.1; edgeProb <= 1.0; edgeProb += 0.1) {
        		for (double p = 0.05; p <= 1.0; p += 0.05) {
        			for (int graphNumber=1; graphNumber<=50; graphNumber++)
        			createAndAdopt(timestepfw, linearfw, graphSize, edgeProb, p, graphNumber, iteration);
        		}
        	}
        }    
     	timestepfw.close();
	...
	private static void createAndAdopt(FileWriter timestepfw,
			FileWriter linearfw, int noNodes, double edgeProb, double p,int graphNumber, int iteration)
			throws IOException {
        ...
        graphState state = new graphState();
     	...
     	do {
         	...
         	linearfw.write(iteration + "," +graphNumber + "," + (state.getYtadd1()-state.getYt()) + "," + 1 + "," + state.getYt() 
         					+ "," + state.getYt()*state.getYt() + "," +edgeProb + "," + p+"\n");
    ...
    private static UndirectedGraph<LabelledNode, DefaultEdge> initRandomGraph(int graphSize, double edgeProb)
    {
